"""
Benchmark for FlexKV distributed KVCache in direct mode (non-server_client_mode).

In direct mode, KVManager creates KVTaskEngine directly in the main process
without going through KVServer/KVDPClient IPC. This is simpler and has lower
overhead, suitable for single-instance single-dp benchmarks.

The key difference from server_client_mode:
  - No KVServer subprocess is spawned
  - KVTaskEngine runs directly in the main process
  - KVTPClient still registers GPU blocks via ZMQ to KVTaskEngine
  - RedisMeta is created directly in KVManager (not inside KVServer)
...
Usage:
  ...
  python benchmarks/dist_benchmark/benchmark_dist_direct.py \\
      --config benchmarks/dist_benchmark/example_dist_direct_config.yml
...
def load_dist_direct_config(config_path: str):
    """Load config for direct mode (non-server_client_mode) distributed KVCache.

    This is similar to load_dist_config in benchmark_dist_kvcache.py, but
    ensures server_client_mode is NOT set, so KVManager uses KVTaskEngine
    directly instead of going through KVServer IPC.
    """
    import yaml
    ...
    if config.get("server_client_mode", False):
        print("[WARN] server_client_mode is set in config but will be IGNORED in direct mode benchmark.")
...
def run_tp_client(dp_client_id, tp_rank, gpu_register_port, model_config, cache_config, num_gpu_blocks):
    ...
    In direct mode, KVTPClient still communicates with KVTaskEngine via ZMQ
    to register GPU memory blocks. The difference is that KVTaskEngine runs
...
def benchmark_single_batch(kvmanager, model_config, cache_config, bench_config):
    """Benchmark single batch put/get with distributed KVCache (direct mode)"""
    print("\n" + "=" * 60)
    print("  Single Batch Benchmark (Distributed KVCache - Direct Mode)")
    print("=" * 60)
...
def benchmark_multiturn(kvmanager, model_config, cache_config, bench_config):
    """Benchmark multi-turn conversation with distributed KVCache (direct mode)"""
    print("\n" + "=" * 60)
    print("  Multi-Turn Conversation Benchmark (Distributed KVCache - Direct Mode)")
    print("=" * 60)
...
def main(args):
    ...
    # Load config (ensures server_client_mode is OFF)
    model_config, cache_config = load_dist_direct_config(args.config)
    ...
    print("=" * 60)
    print("  FlexKV Distributed KVCache Benchmark (Direct Mode)")
    print("=" * 60)
    ...
    print(f"  server_client_mode: False (direct mode)")

    # Create KVManager in direct mode
    # In direct mode, KVManager creates KVTaskEngine directly (no KVServer subprocess)
    # RedisMeta is also created directly in KVManager
    kvmanager = KVManager(model_config, cache_config)
    kvmanager.start()

    # Verify we are indeed in direct mode
    assert not kvmanager.server_client_mode, \
        "Expected direct mode (server_client_mode=False), but got server_client_mode=True. " \
        "Check your config: dp_size must be 1, instance_num must be 1, and " \
        "FLEXKV_SERVER_CLIENT_MODE env var must not be set."

    # Start tp_client processes to register GPU blocks
    # Even in direct mode, GPU blocks are registered via KVTPClient -> KVTaskEngine ZMQ
    tp_client_processes = []
    ...
    print("\nWaiting for FlexKV to be ready (direct mode)...")
    ...
    try:
        ...
        # Print summary
        print("\n" + "=" * 60)
        print("  Benchmark Summary (Direct Mode)")
        print("=" * 60)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Benchmark FlexKV distributed KVCache in direct mode (non-server_client_mode)"
    )
    parser.add_argument("--config", type=str,
                        default="benchmarks/dist_benchmark/example_dist_direct_config.yml",
