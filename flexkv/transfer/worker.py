...
class GPUCPUTransferWorker(TransferWorkerBase):  # this worker only supports non-tp and non-dp case
    def __init__(self,
        ...
        self.packed_kv = gpu_kv_layout.packed_kv
...
class CPULakeTransferWorker(TransferWorkerBase):
    def __init__(self,
        ...
        if self.has_multi_group:
            if (
                cpu_kv_layout.type != KVCacheLayoutType.BLOCKFIRST
                or lake_kv_layout.type != KVCacheLayoutType.BLOCKFIRST
            ):
                raise ValueError(
                    "Multi-group CPU/lake transfer requires BLOCKFIRST layouts"
                )
            # A heterogeneous BLOCKFIRST block is already one byte-flat blob.
            # Present it to the existing lake kernel as one MLA layer.
        ...
        self.lake_layer_stride_in_bytes = (
            self.num_lake_blocks * self.block_size * self.dtype.itemsize * self.kv_dim
        )
...
class PEER2CPUTransferWorker(TransferWorkerBase):
    def __init__(self,
        ...
        self.peer_kv_layout = peer_kv_layout
    ...
    def get_cpu_buffer_block_start_ptr(
        ...
        # Assume that the peer CPU buffer layout matches the local CPU layout.
        assert self.cpu_kv_layout.type == self.peer_kv_layout.type
        src_block_ptrs = []
...
class MooncakeStoreTransferWorker(TransferWorkerBase):
    """Mooncake-store lake KV I/O worker (main KV and SWA pools)."""
    ...
    def _transfer_impl(self, cpu_ptrs, block_sizes, keys, transfer_type: TransferType) -> None:
        if transfer_type == TransferType.H2LAKE:
            ...
            raise ValueError(
                f"MooncakeStoreTransferWorker only supports H2LAKE/LAKE2H, got {transfer_type}")
    ...
    def _preprocess_kv(self, transfer_op: WorkerTransferOp):
        cpu_block_ids = (
            ...
            if transfer_op.transfer_type == TransferType.LAKE2H
